/*
 * irc — DOS/9 IRC client.
 *
 * Usage:
 *   irc <host> [port] [nick] [channel]
 *
 * The client resolves hostnames via /net/resolve, opens a TCP connection
 * through /net/tcp, sends the IRC registration handshake, optionally joins a
 * channel, and then prints the server response verbatim.
 */
...
static int irc_session(const char *host_ip, int port, const char *nick,
                       const char *channel) {
    int fd = tcp_connect(host_ip, port);
    if (fd < 0) return -1;

    if (!nick || !*nick) nick = "dos9";
    char line[256];
    int len = 0;

    if (append_str(line, &len, (int)sizeof(line), "NICK ") < 0 ||
        append_str(line, &len, (int)sizeof(line), nick) < 0 ||
        append_char(line, &len, (int)sizeof(line), '\r') < 0 ||
        append_char(line, &len, (int)sizeof(line), '\n') < 0) {
        close(fd);
        return -1;
    }
    if (send_all(fd, line, len) < 0) { close(fd); return -1; }

    len = 0;
    if (append_str(line, &len, (int)sizeof(line), "USER ") < 0 ||
        append_str(line, &len, (int)sizeof(line), nick) < 0 ||
        append_str(line, &len, (int)sizeof(line), " 0 * :DOS/9 IRC") < 0 ||
        append_char(line, &len, (int)sizeof(line), '\r') < 0 ||
        append_char(line, &len, (int)sizeof(line), '\n') < 0) {
        close(fd);
        return -1;
    ...
    if (channel && *channel) {
        len = 0;
        if (append_str(line, &len, (int)sizeof(line), "JOIN ") < 0 ||
            append_str(line, &len, (int)sizeof(line), channel) < 0 ||
            append_char(line, &len, (int)sizeof(line), '\r') < 0 ||
            append_char(line, &len, (int)sizeof(line), '\n') < 0) {
            close(fd);
            return -1;
...
static void usage(void) {
    puts("usage: irc <host> [port] [nick] [channel]");
    puts("  connect to an IRC server and print the session transcript");
}
...
int main(int argc, const char **argv) {
    ...
    const char *nick = "dos9";
    const char *channel = "";

    if (argc >= 3) {
        if (parse_port(argv[2], &port) < 0) {
            puts("irc: invalid port");
            return 1;
        }
    }
    ...
    if (is_hostname(host)) {
        if (resolve_hostname(host, ip_buf, (int)sizeof(ip_buf)) < 0) {
            puts("irc: hostname resolution failed");
            return 1;
        }
        ip = ip_buf;
    }

    if (irc_session(ip, port, nick, channel) < 0) {
        puts("irc: session failed");
        return 1;
    }
