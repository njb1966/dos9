/*
 * finger — DOS/9 Finger client.
 *
 * Usage:
 *   finger <host> [port] [query]
 *
 * The query defaults to the empty string, which asks the server for a
 * directory-style listing. The client resolves hostnames via /net/resolve,
 * opens a TCP connection through /net/tcp, sends the request line, and then
 * prints the server response verbatim.
 */

#include <dos9.h>

#define FINGER_DEFAULT_PORT 79
...
static int finger_connect(const char *host_ip, int port) {
    char slot_buf[4] = {0};
...
static int finger_fetch(const char *host_ip, int port, const char *query) {
    int fd = finger_connect(host_ip, port);
    if (fd < 0) return -1;

    if (!query) query = "";
    if (send_all(fd, query, (int)strlen(query)) < 0 ||
        send_all(fd, "\r\n", 2) < 0) {
        close(fd);
        return -1;
    }
...
static void usage(void) {
    puts("usage: finger <host> [port] [query]");
    puts("  fetch a Finger response over plain TCP and print it");
}
...
int main(int argc, const char **argv) {
    ...
    int port = FINGER_DEFAULT_PORT;
    const char *query = "";

    if (argc >= 3) {
        if (parse_port(argv[2], &port) < 0) {
            puts("finger: invalid port");
            return 1;
        }
    }
    if (argc >= 4) query = argv[3];

    char ip_buf[20];
    const char *ip = host;
    if (is_hostname(host)) {
        if (resolve_hostname(host, ip_buf, (int)sizeof(ip_buf)) < 0) {
            puts("finger: hostname resolution failed");
            return 1;
        }
        ip = ip_buf;
    }

    if (finger_fetch(ip, port, query) < 0) {
        puts("finger: fetch failed");
        return 1;
    }
